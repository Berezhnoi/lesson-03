...
Make the changes directly in the repository. Return only Go code in three blocks: the `OrderStore` interface, the refactored `OrderService`, and the fake store used for tests. Put tests for the service in a separate `orders_ai_test.go` file. After the code, briefly report what changed and the test result.
